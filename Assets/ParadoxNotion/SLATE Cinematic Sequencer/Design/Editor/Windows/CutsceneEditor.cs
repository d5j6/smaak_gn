...
namespace Slate{
	...
	public class CutsceneEditor : EditorWindow{
		...
		//Layout variables
		private float leftMargin{ //margin on the left side. Basicaly the width of the group/tracks list.
			get {return Prefs.trackListLeftMargin;}
			set {Prefs.trackListLeftMargin = Mathf.Clamp(value, 230, 400);}
		}
		private readonly float rightMargin         = 16; //margin on the right side
		private readonly float toolbarHeight       = 18; //the height of the toolbar
		private readonly float topMargin           = 40; //top margin AFTER the toolbar
		private readonly float groupHeight         = 21; //height of group headers
		private readonly float trackMargins        = 4;  //margin between tracks of same group (topb/bottom)
		private readonly float groupRightMargin    = 4;  //margin at the right side of groups
		private readonly float trackRightMargin    = 4;  //margin at the right side of tracks
		private readonly float firstGroupTopMargin = 20; //initial top margin
		//
		...
		private const float CLIPS_SNAP_INTERVAL = 0.1f;
		...
		void OnGUI(){
			...
			topMiddleRect     = new Rect(leftMargin, toolbarHeight, screenWidth - leftMargin - rightMargin, topMargin);
			leftRect          = new Rect(0, toolbarHeight + topMargin, leftMargin, screenHeight - toolbarHeight - topMargin + scrollPos.y);
			centerRect        = new Rect(leftMargin, topMargin + toolbarHeight, screenWidth - leftMargin - rightMargin, screenHeight - toolbarHeight - topMargin + scrollPos.y);
			//topRightRect    = new Rect(screenWidth - rightMargin, toolbarHeight, rightMargin, topMargin);
			//rightRect       = new Rect(screenWidth - rightMargin, topMargin, rightMargin, totalHeight);
			...
			if (showDragDropInfo && cutscene.groups.Find(g => g.GetType() == typeof(ActorGroup)) == null){
				...
				notificationRect.center = new Vector2((screenWidth/2) + (leftMargin/2), (screenHeight/2) + topMargin);
		...
		void ShowToolbar(){
			...
			if (GUILayout.Button(Slate.Styles.gearIcon, EditorStyles.toolbarButton, GUILayout.Width(26))){
				PreferencesWindow.Show(new Rect(screenWidth - 5 - 400, toolbarHeight + 5, 400, screenHeight - toolbarHeight - 50));
			}
		...
		void ShowGroupsAndTracksList(Rect leftRect){
			...
			var nextYPos = firstGroupTopMargin;
			var wasEnabled = GUI.enabled;
			GUI.enabled = true;
		...
		void ShowTimeLines(Rect centerRect){
			...
			//bg graphic
			var bgRect = Rect.MinMaxRect(centerRect.xMin, topMargin + toolbarHeight + scrollPos.y, centerRect.xMax, screenHeight - toolbarHeight + scrollPos.y);
					...
					for (int a= 0; a < track.actions.Count; a++){
						...
						if (anyClipDragging && ReferenceEquals(CutsceneUtility.selectedObject, action) ){
							...
							if ( multiSelection == null || multiSelection.Count < 1 ){
								...
								if (Mathf.Abs(xTime - preTime) <= CLIPS_SNAP_INTERVAL){
									xTime = preTime;
								}
								if (Mathf.Abs(xTime - postTime) <= CLIPS_SNAP_INTERVAL){
									xTime = postTime;
								}
		...
		class ActionClipWrapper{
			...
			public void OnClipGUI(){
				...
				if (isScalingStart){
					...
					if (Mathf.Abs(snapedPointerTime - prev) <= CLIPS_SNAP_INTERVAL){
				...
				if (isScalingEnd){
					...
					if (Mathf.Abs(snapedPointerTime - next) <= CLIPS_SNAP_INTERVAL){
