...
namespace Slate{
	...
	public class AnimatedParameter : IAnimatableData{
		...
		public void Evaluate(float time, float previousTime){

			//evaluate curves first
			if (!Application.isPlaying){
				#if UNITY_EDITOR
				Internal_Evaluate_Editor(time, previousTime);
				#endif
			} else {
				Internal_Evaluate_Runtime(time, previousTime);
			}
		...
		public void CompileExpression(){
			...
			var env = keyable.GetExpressionEnvironment().Push();
			Slate.Expressions.ExpressionParameterWrapper.Wrap(this, env);
			//...

			try
			{
				compiledExpression = StagPoint.Eval.EvalEngine.Compile(scriptExpression, env);
				...
				var valueType = compiledExpression.Execute().GetType();
...
#if UNITY_EDITOR
		////EDITOR EVAL////
		void Internal_Evaluate_Editor(float time, float previousTime){
		...
		void Internal_Evaluate_Runtime(float time, float previousTime){
