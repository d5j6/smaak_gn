...
namespace Slate{
	...
	public class DirectorGUI : MonoBehaviour {
		...
		void DoLetterbox(){
			...
			a.y = Mathf.Lerp(-cineboxSize, 0, lerp);
			b.y = Mathf.Lerp(Screen.height, Screen.height - cineboxSize, lerp);
		...
		void DoSubs(){
			var finalSubs = string.Format("<size={0}><b><i>{1}</i></b></size>", subtitleSize, subsText);
